...
DEFAULT_CONFIG = {
    ...
    "auxiliary": {
        ...
        #
        # Defaults to true, paired with a :free openrouter_model below.
        # Background auxiliary work should not silently bill: the previous
        # default engaged a paid lane for routine compression and title
        # generation on any install that never set these keys. Set
        # free_only: false to restore the historical paid fallback.
        #
        # These two keys must move together. free_only: true with a paid
        # openrouter_model does not fall back to a free one — it skips the
        # OpenRouter step entirely, silently removing a fallback lane.
        "free_only": True,
        # The auxiliary auto-chain's OpenRouter fallback model. Must be a
        # ":free" SKU while free_only is true (see above), otherwise the
        # OpenRouter fallback is skipped. A one-time WARNING is logged
        # whenever a non-":free" model is engaged.
